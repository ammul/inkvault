...
function load() {
  ...
  if (entry) toast.show(t('diary.entryLoaded'))
}
...
<template>
  <div class="space-y-5">
    ...
    <div v-if="datapoints.configs.length" class="space-y-2">
      <h3 class="text-xs font-medium text-ink-faint uppercase tracking-widest">{{ t('diary.dataPointsSection') }}</h3>
      <DataPointField
    ...
    <div>
      ...
      >
        {{ saving ? t('diary.saving') : t('diary.saveEntry') }}
      </button>
    </div>
  </div>
</template>
