...
const props = defineProps<{ initial?: DataPointConfig; locked?: boolean; compact?: boolean }>()
...
function defaultSample(t: DataPointType): DataPointValue {
  switch (t) {
...
function initialSample(): DataPointValue {
...
const sampleValue = ref<DataPointValue>(initialSample())

const liveConfig = computed<DataPointConfig>(() => ({
  id: props.initial?.id ?? '__preview__',
  createdAt: props.initial?.createdAt ?? '',
  label: label.value || t('dataPoints.editor.labelPlaceholder'),
  color: color.value,
  icon: icon.value,
  type: type.value,
  config: buildConfig(),
...
<template>
  ...
  >
    <h3 v-if="!compact" class="font-semibold text-ink">
      {{ initial ? t('dataPoints.editor.editTitle') : t('dataPoints.editor.newTitle') }}
    </h3>
    ...
    <div :class="compact ? 'grid grid-cols-1 md:grid-cols-[1.05fr_0.95fr]' : 'space-y-4'">
      ...
      <div :class="compact ? 'p-4 space-y-3' : 'space-y-4'">
        ...
        <div>
          <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.label') }}</label>
          <input
            v-model="label"
            type="text"
        ...
        <div class="flex gap-4 items-end flex-wrap">
          <div v-if="appSettings.settings.useEmojis">
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.icon') }}</label>
            <EmojiPicker v-model="icon" />
          </div>
          <div>
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.color') }}</label>
            <div
              role="radiogroup"
              :aria-label="t('dataPoints.editor.colorAriaLabel')"
            ...
            >
              ...
              >{{ t('dataPoints.editor.colorCustom') }}</button>
              <input
                ref="customColorRef"
                v-model="color"
                type="color"
        ...
        <!-- Type picker -->
        <div>
          <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.type') }}</label>
        ...
        <div v-if="type === 'range'" class="grid grid-cols-3 gap-2" :class="{ 'opacity-60 select-none': locked }">
          <div>
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.range.min') }}</label>
            <input
          ...
          <div>
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.range.max') }}</label>
            <input
          ...
          <div>
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.range.step') }}</label>
            <input
          ...
          <p v-if="locked" class="col-span-3 text-[11px] text-ink-faint">{{ t('dataPoints.editor.lockedFieldHint') }}</p>
        </div>

        <!-- String field -->
        <div v-if="type === 'string'" :class="{ 'opacity-60 select-none': locked }">
          <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.string.placeholder') }}</label>
          <input
            v-model="placeholder"
            type="text"
            :disabled="locked"
            ...
            class="w-full border border-edge rounded-input px-3 py-2 text-sm text-ink bg-surface"
          />
          <p v-if="locked" class="text-[11px] text-ink-faint mt-1">{{ t('dataPoints.editor.lockedFieldHint') }}</p>
        </div>

        <!-- Multi-string field -->
        <div v-if="type === 'multi-string'">
          <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.multiString.options') }}</label>
          <!-- Locked: chips -->
          ...
          <p class="text-[11px] text-ink-faint mt-1">
            {{ locked ? t('dataPoints.editor.lockedFieldHint') : t('dataPoints.editor.multiString.optionsHint') }}
          </p>
        </div>
        ...
        <div v-if="type === 'boolean'" class="grid grid-cols-2 gap-2" :class="{ 'opacity-60 select-none': locked }">
          <div>
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.boolean.trueLabel') }}</label>
            <input
              v-model="trueLabel"
              type="text"
              :disabled="locked"
          ...
          <div>
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.boolean.falseLabel') }}</label>
            <input
              v-model="falseLabel"
              type="text"
              :disabled="locked"
          ...
          <p v-if="locked" class="col-span-2 text-[11px] text-ink-faint">{{ t('dataPoints.editor.lockedFieldHint') }}</p>
        </div>

        <!-- Medication fields -->
        <div v-if="type === 'medication'" class="space-y-3" :class="{ 'opacity-60 select-none': locked }">
          <div>
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.medication.name') }}</label>
            <input
          ...
          <div>
            <label class="block text-[11px] font-medium text-ink-muted mb-1.5">{{ t('dataPoints.editor.medication.dosagePresets') }}</label>
            <input
          ...
          <p v-if="locked" class="text-[11px] text-ink-faint">{{ t('dataPoints.editor.lockedFieldHint') }}</p>
        </div>
      ...
      <div v-if="compact" class="p-4 bg-subtle border-t border-edge md:border-t-0 md:border-l md:border-edge">
        <div class="text-[10px] font-semibold uppercase tracking-wider text-ink-faint flex items-center gap-1.5 mb-3">
          ...
          {{ t('dataPoints.preview.livePreview') }}
        </div>
        <DataPointField :config="liveConfig" v-model="sampleValue" />
      </div>
